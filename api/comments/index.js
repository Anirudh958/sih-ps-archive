...
import { json, methodNotAllowed, parseCookies, validOrigin } from "../../lib/http.js";
...
export default async function handler(request, response) {
  ...
  if (!session?.groupId) return json(response, 403, { error: "Join a group to use comments" });
  ...
  const displayName = parseCookies(request).sih_name || "Team member";
