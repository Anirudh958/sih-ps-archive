...
async function main() {
  ...
  await expectStatus(outsiderClient.request("/api/comments?ps=SIH26011", { origin: false }), 403, "non-member cannot read comments");
  await expectStatus(ownerClient.request("/api/comments?ps=SIH26011", {
    method: "POST",
