...
export async function verifyAccess(request) {
  ...
  try {
    ...
    const sql = db();
    const rows = await sql`SELECT group_key FROM browse_sessions WHERE id = ${payload.sub} AND revoked_at IS NULL AND expires_at > NOW()`;
    return rows.length ? { sessionId: payload.sub, groupId: rows[0].group_key || "" } : null;
  } catch {
    return null;
  }
}

export async function issueSession(response, captchaToken, metadata = {}) {
  ...
  return { accessToken: data.session.access_token, expiresIn: data.session.expires_in, groupJoined: false };
}

export async function joinSession(sessionId, groupId) {
  const sql = db();
  const rows = await sql`UPDATE browse_sessions SET group_key = ${groupId} WHERE id = ${sessionId} AND revoked_at IS NULL AND expires_at > NOW() RETURNING id`;
...
export async function rotateSession(request, response) {
  ...
  setRefreshCookie(response, data.session.refresh_token);
  return { accessToken: data.session.access_token, expiresIn: data.session.expires_in, groupJoined: Boolean(rows[0].group_key) };
}
